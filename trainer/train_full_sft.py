"""
MiniMind 全参监督微调脚本（四阶段训练：第 2 阶段）
==================================================
在预训练权重基础上做 SFT：让模型学会"只预测 assistant 回复"。
支持混合精度、梯度累积、余弦退火 lr、断点续训、swanlab 实验曲线、KDA 架构选择。

用法示例：
  # 基于预训练权重做 SFT（KDA hybrid 架构）
  python train_full_sft.py --epochs 2 --batch_size 16 --max_seq_len 768
...
# ==========================================================================================
#  与预训练循环几乎一致，区别仅在数据（SFT 的 labels 只有 assistant 回复部分有效）。
#  核心流程（每个 step）：
#    1. 数据 → GPU
#    2. 更新学习率（余弦退火）
...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        with autocast_ctx:
            ...
            # SFT 的 labels 大部分位置是 -100（只保留 assistant 回复），
            # CrossEntropyLoss 自动忽略这些位置
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_weight", default="full_sft", type=str, help="保存权重文件名前缀")
    parser.add_argument("--epochs", type=int, default=2, help="训练轮数")
    parser.add_argument("--batch_size", type=int, default=16, help="每 GPU 的 batch size")
    parser.add_argument("--learning_rate", type=float, default=1e-5,
                        help="初始学习率（SFT 比预训练低约两个数量级，避免遗忘预训练知识）")
    ...
    # -- KDA 注意力架构（需与预训练基座保持一致） --
    parser.add_argument("--attn_type", default="hybrid", type=str,
                        choices=["softmax", "kda", "hybrid"],
                        help="注意力类型，必须与预训练基座一致：softmax=全注意力；kda=全 KDA；hybrid=3:1 混合")
    ...
    parser.add_argument("--data_path", type=str,
                        ...
                        help="SFT 数据路径（JSONL 格式，每行 {\"conversations\": [...]}）")
    parser.add_argument("--from_weight", default="pretrain", type=str,
                        help="基于哪个权重训练（'none'=不基于任何权重）")
    ...
    # SFTDataset: 返回 (input_ids, labels)，labels 只有 assistant 回复部分有效
